...
class streamWrapperHttpAuth
{
    ...
    protected function postRequest($path, $authType = CURLAUTH_ANY, $attempted=array())
    {
        FB::log(__FUNCTION__);
        FB::log('$authType: ' . $authType);
        FB::log('CURLAUTH_ANY: ' . CURLAUTH_ANY);
        FB::log('CURLAUTH_NTLM: ' . CURLAUTH_NTLM);
        FB::log('CURLAUTH_BASIC: ' . CURLAUTH_BASIC);
        ...
        $attempted[$authType] = $info['http_code'];
        FB::log($info);
        FB::log($attempted);
        ...
        { 
            FB::log('401 error on request');
            // Attempt NTLM Auth only, CURLAUTH_ANY does not work with NTML
            $key = CURLAUTH_NTLM;
            if (!array_key_exists($key, $attempted)) {
                return $this->postRequest($path, $key, $attempted);
            }
